...
function findGifUrl(exerciseName) {
  ...
  for (const [key, url] of Object.entries(EXERCISE_GIFS)) {
    ...
    if (score > bestScore && score >= 0.5) { bestScore = score; bestMatch = url; }
  }
  return bestMatch;
}
...
Deno.serve(async (req) => {
  try {
    ...
    // This function is called by automation (no user auth needed) or by admin/personal
    // Check for payload from entity automation
    let exerciseId = null;
    ...
    const sourceUrl = findGifUrl(exerciseName);
    if (!sourceUrl) {
      return Response.json({ success: true, found: false, exercise_name: exerciseName });
    }

    const hostedUrl = await downloadAndUpload(sourceUrl, base44);
    if (!hostedUrl) {
      return Response.json({ success: true, found: false, reason: "download failed" });
    }

    await base44.asServiceRole.entities.Exercise.update(exerciseId, { image_url: hostedUrl });

    return Response.json({ success: true, found: true, gif_url: hostedUrl, exercise_name: exerciseName });

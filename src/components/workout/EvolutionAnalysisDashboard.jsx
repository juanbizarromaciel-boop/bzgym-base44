...
export default function EvolutionAnalysisDashboard({ report, generatedPlans, selectedPlans, activeTab, setActiveTab, updateGeneratedPlan, updateGeneratedExercise, removeGeneratedExercise, addGeneratedExercise, onSaveDraft, onPreview, onApply, onExportPdf, onExportHtml, onExportWhatsApp, saving, onCancel }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="flex gap-2 overflow-x-auto pb-1">{tabs.map(t => <button key={t} onClick={() => setActiveTab(t)} className={`px-3 py-2 rounded-lg text-xs border whitespace-nowrap ${activeTab === t ? "border-cyan-400/50 bg-cyan-400/10 text-cyan-100" : "border-purple-900/30 text-purple-200/60"}`}>{labels[t] || report.analisePorTreino[Number(t)]?.nome || `Treino ${Number(t) + 1}`}</button>)}</div>
    ...
    <div className="grid sm:grid-cols-5 gap-2 pt-2"><Button onClick={onSaveDraft} disabled={saving} variant="outline"><Save className="w-4 h-4 mr-2" />Salvar rascunho</Button><Button onClick={() => onExportPdf("claro")} className="btn-neon-purple"><Download className="w-4 h-4 mr-2" />Exportar PDF Premium</Button><Button onClick={() => setActiveTab("relatorio")} className="btn-neon-cyan"><FileText className="w-4 h-4 mr-2" />Ver prévia</Button><Button onClick={onApply} disabled={saving} className="btn-neon-purple"><CheckCircle2 className="w-4 h-4 mr-2" />Aplicar</Button><Button variant="outline" onClick={onCancel}>Cancelar</Button></div>
  </div>;
}
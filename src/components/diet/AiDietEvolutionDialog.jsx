...
export default function AiDietEvolutionDialog({ open, onOpenChange, plan, owner, currentUser, allPlans = [], selfMode = false, onApplied }) {
  ...
  const applyDiet = async () => {
    ...
    if (!selfMode && owner?.email) await base44.entities.Notificacao.create({ usuario_id: owner.email, titulo: "Sua dieta foi atualizada", mensagem: "Uma nova versão da sua dieta foi aplicada.", tipo: "dieta_atualizada", lida: false, link_destino: "/MyDiet", icone: "Utensils" });
    setAppliedPlan(newPlan);
    setStep(6);
    setSaving(false); toast.success("Nova dieta aplicada com histórico preservado."); onApplied?.();
  };
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="text-white border border-emerald-900/40 w-[96vw] sm:max-w-5xl max-h-[88dvh] overflow-y-auto p-3 sm:p-6" style={{ background: "#04040e" }}>
      <DialogHeader><DialogTitle className="font-cyber tracking-widest text-emerald-200 flex items-center gap-2 text-sm sm:text-base"><Sparkles className="w-5 h-5 text-cyan-300" /> EVOLUIR DIETA COM IA</DialogTitle></DialogHeader>
      {appliedPlan ? <div className="rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-6 text-center space-y-4">
        <CheckCircle2 className="w-14 h-14 mx-auto text-emerald-300" />
        <h2 className="font-cyber text-xl text-white tracking-widest">DIETA TROCADA COM SUCESSO</h2>
        <p className="text-emerald-100/80">A nova dieta <b>{appliedPlan.name}</b> foi aplicada, a dieta antiga foi arquivada como substituída e o histórico foi preservado.</p>
        {!selfMode && owner?.email && <p className="text-sm text-purple-100/65">O aluno recebeu uma notificação em “Minha Dieta”.</p>}
        <div className="grid sm:grid-cols-3 gap-2"><Button onClick={() => exportPdf("claro")} className="btn-neon-purple">Exportar PDF</Button><Button onClick={exportWhatsApp} variant="outline">Copiar WhatsApp</Button><Button onClick={() => onOpenChange(false)} className="btn-neon-cyan">Fechar</Button></div>
      </div> : !allowed ? <div className="p-6 rounded-xl border border-amber-500/20 bg-amber-500/5 text-amber-200">Seu perfil não tem permissão para evoluir esta dieta.</div> : <div className="space-y-4 sm:space-y-5">
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-1.5 sm:gap-2">{steps.map((s, i) => <button key={s} onClick={() => i <= step && setStep(i)} className={`rounded-lg sm:rounded-xl px-2 sm:px-3 py-2 text-[10px] sm:text-xs border ${i === step ? "border-emerald-400/50 bg-emerald-400/10 text-emerald-200" : "border-purple-900/30 bg-purple-500/5 text-purple-300/60"}`}>{i + 1}. {s}</button>)}</div>

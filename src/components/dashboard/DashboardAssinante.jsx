...
export default function DashboardAssinante({ user }) {
  ...
  return <DashboardBase user={user} roleLabel="Assinante"><DashboardPrimaryCard icon={Sparkles} eyebrow="BZI" title="Seus recursos premium" subtitle="Treino, nutrição e conteúdos liberados no seu plano" path="/AICoach" actionLabel="Explorar" /><DashboardLinkGrid title="Recursos disponíveis" items={items} /><DashboardPlanCard status={user?.assinatura_status} dueDate={user?.assinatura_vencimento} /><section><h2 className="app-section-title mb-2.5 text-[13px]">IA de macros</h2><div className="app-glass-card rounded-[18px] p-3"><MacroAI /></div></section></DashboardBase>;
}
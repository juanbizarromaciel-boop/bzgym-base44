...
export default function AuthenticatedApp() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const subscriber = profile.account_type === "assinante" || profile.assinatura_status || profile.assinatura_vencimento || profile.assinatura_origem || profile.stripe_subscription_id;
      const role = subscriber && !["admin", "personal", "recente", "bloqueado"].includes(baseRole) ? "assinante" : baseRole;
  ...
  const accessState = blocked(user) ? "blocked" : user?.role === "recente" || (user?.role === "user" && !student) ? "onboarding" : user?.role === "user" && student?.active === false ? "pending" : "active";

...
import { format, startOfDay, startOfWeek, startOfMonth, subDays, subWeeks, subMonths, parseISO } from "date-fns";
...
function calcVolume(log) {
  ...
  // fallback: use max_load_kg if no sets data
...
export default function Progress() {
  ...
  const studentLogs = useMemo(() => {
    return logs
      .filter((l) => l.student_id === selectedStudentId)
  ...
  const exerciseNames = useMemo(() => {
    const names = new Set(studentLogs.map((l) => l.exercise_name));
    return [...names].filter(Boolean).sort();
  }, [studentLogs]);

  // Map exercise name -> muscle group
  const exerciseMuscleMap = useMemo(() => {
    const map = {};
    exercises.forEach(ex => { map[ex.name] = ex.muscle_group; });
    // also from plans
    allStudentExercises.forEach(ex => {
      if (ex.exercise_name && !map[ex.exercise_name]) {
        map[ex.exercise_name] = ex.muscle_group;
      }
    });
    return map;
  }, [exercises, allStudentExercises]);

  // Filtered logs by exercise and muscle
  const filteredLogs = useMemo(() => {
    let f = studentLogs;
    if (selectedExercise !== "all") f = f.filter(l => l.exercise_name === selectedExercise);
    if (selectedMuscle !== "all") {
      f = f.filter(l => (exerciseMuscleMap[l.exercise_name] || "outro") === selectedMuscle);
    }
    return f;
  ...
  const chartData = useMemo(() => {
    ...
    if (period === "daily") {
      ...
      filteredLogs.forEach(log => {
        ...
        if (buckets[key]) {
          buckets[key].volume += calcVolume(log);
          buckets[key].carga = Math.max(buckets[key].carga, log.max_load_kg || 0);
          buckets[key].count++;
        }
      });
    ...
    return (
      <div className="border rounded-xl px-4 py-3 shadow-xl" style={{background: '#04040e', borderColor: 'rgba(168,85,247,0.4)', boxShadow: '0 0 20px rgba(168,85,247,0.15)'}}>
        ...
        {payload[0]?.payload?.count > 0 && (
          <p className="text-xs text-purple-400/30 font-mono-cyber mt-1">{payload[0].payload.count} registros</p>
        )}
      </div>
    );
  };
          ...
          <div className="flex rounded-lg border border-purple-900/30 overflow-hidden">
            {PERIOD_OPTIONS.map(opt => (
              <button
                ...
                className={`px-4 py-2 text-xs font-mono-cyber tracking-wider transition-all ${
                  period === opt.value
                    ? "bg-purple-500/20 text-purple-300 border-r border-purple-900/30"
                    : "text-purple-500/40 hover:text-purple-400 hover:bg-purple-500/10 border-r border-purple-900/30"
                } last:border-r-0`}
              >
                {opt.label}
              </button>
                ...
                return (
                  <div key={muscle} className="space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-cyber text-sm" style={{ color, textShadow: `0 0 8px ${color}60` }}>
                ...
                return (
                  <div key={ex.name} className="flex items-center justify-between px-4 py-3 rounded-lg bg-black/40 border border-purple-900/20 hover:border-purple-500/20 transition-all">
                    ...
                    <div className="text-right">
                      <p className="font-cyber text-lg text-purple-300" style={{ textShadow: '0 0 8px rgba(168,85,247,0.5)' }}>
